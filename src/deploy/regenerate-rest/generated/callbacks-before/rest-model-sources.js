...
module.exports = async function(args) {
	try {
		...
		for (let index = 0; index < tableNames.length; index++) {
			...
			const parameters = { schema, table, tableData };
			const templates = await Promise.all([
				new Promise(function(ok, fail) {
					ejs.renderFile(commonPaths.actor, parameters, asynchandler(ok, fail));
				}),
				new Promise(function(ok, fail) {
					ejs.renderFile(commonPaths.controller, parameters, asynchandler(ok, fail));
				}),
				new Promise(function(ok, fail) {
					ejs.renderFile(commonPaths.middleware, parameters, asynchandler(ok, fail));
				}),
				new Promise(function(ok, fail) {
					ejs.renderFile(commonPaths.baseActor, parameters, asynchandler(ok, fail));
				}),
				new Promise(function(ok, fail) {
					ejs.renderFile(commonPaths.baseController, parameters, asynchandler(ok, fail));
				}),
				new Promise(function(ok, fail) {
					ejs.renderFile(commonPaths.baseMiddleware, parameters, asynchandler(ok, fail));
				}),
			]);
