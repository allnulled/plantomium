...
module.exports = async function(argv) {
	...
	try {
		...
		cms.utils.hasOnlyKeys(args, ["_", "model", "where", "limit"]);
		...
		const getResponse = await client.api[args.model].deleteMany(args);
		...
		if(error) {
			...
			cms.utils.printSqlData(data.operations, true);
			console.log("[Items:]");
			cms.utils.printSqlData(data.items || [], true);
			console.log("[Attachments:]");
			cms.utils.printSqlData(data.attachments || [], true);
		}
